...
function sendEmbeds(text, message) {
  ...
  for (let chunk of arr) {
    let embed = new EmbedBuilder().setColor(0x36393f).setDescription(chunk);
...
module.exports = {
  ...
  run: async (message, client) => {
    ...
    if (message.author.bot || !message.content.startsWith("-") || !data) return;
    ...
    sendEmbeds(response, message || "No Response");
    rcon.close();
  },
};
